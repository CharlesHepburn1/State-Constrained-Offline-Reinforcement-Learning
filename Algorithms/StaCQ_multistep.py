...
class Agent(object):
    ...
    def train2(self, replay_buffer, list_idx, forward_model, inverse_model, iterations=1):
        ...
        for it in range(iterations):
            ...
            done_traj = torch.index_select(replay_buffer[5], 0, indices).to(self.device)
            done = torch.index_select(replay_buffer[4], 0, indices).to(self.device)
            #mask the indices where done_traj = 1 and done = 0
            mask = ((done_traj ==1) & (done == 0)).detach().cpu()
            indices = torch.masked_select(indices, mask.logical_not())
            ...
            r_tilde = self.Rew.forward(state1, next_state1).reshape(-1, )
            rew_loss = F.mse_loss(reward1, r_tilde)
