...
class InverseModel2(nn.Module):
    def __init__(self, state_dim, action_dim,max_action, hidden_dim=256):
        super(InverseModel2, self).__init__()
    ...
    def forward(self, state, next_state):
        ...
        action = F.relu(self.l4(action))
        return self.max_action * torch.tanh(self.l5(action))



class DetInvDynamics(object):
    def __init__(self, state_dim, action_dim, max_action, lr=3e-4, hidden_dim = 256,device="cpu"):

        self.InverseModel = InverseModel2(state_dim, action_dim,max_action,  hidden_dim = hidden_dim).to(device)
